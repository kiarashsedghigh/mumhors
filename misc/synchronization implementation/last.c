...
static int perform_rejection_sampling(const unsigned char *message, int message_len, int k, int t,
    ...
    /* Hash one time */
    unsigned char hash_ctr_buffer[SHA256_OUTPUT_LEN + sizeof(unsigned int)];
...
static void change_pk_status_from_doubt_with_start(mumhors_verifier_t *verifier, public_key_row_t *start_row, int start_col,
                                                   public_key_row_t *end_row, int end_col, int status, int cnt_pks) {

    // public_key_row_t *start_row = verifier->pk_matrix.head;

    /* Maximum index that we can iterate in each row */
    // int start_index = verifier->c;
    ...
    while (1) {
        ...
        for (int i = start_col; i < end_col_index; i++) {
            if (start_row->pks[i]->type == MUM_PK_DOUBT) {
...
static void debug_pk(mumhors_verifier_t *verifier) {
    ...
    int ss = 0;
    rt = 11;
    pk_row_start = verifier->pk_matrix.head;
    while (pk_row_start) {
        rt--;
        ss += pk_row_start->available_pks;
        // printf("%d\n",pk_row_start->available_pks);
        pk_row_start = pk_row_start->next;
        if (!rt)
            break;
    }

    printf("Total %d / %d \n\n", ss, verifier->active_pks);
}


void get_available_pk_until_index(public_key_row_t *current_row, int current_index, int* available_pks, int* available_doubt_pks) {

    *available_pks = 0;
    *available_doubt_pks = 0;

    // if (current_index == 1023)
    //     return;

    for(int i=current_index ;i<1024 ; i++) {
        // printf("%d\n", i);
        if (current_row->pks[i]->type == MUM_PK_VALID)
            *available_pks += 1;
        else if(current_row->pks[i]->type == MUM_PK_DOUBT)
            *available_doubt_pks += 1;
    }
    // return cnt;
}
...
static int verify_signature_using_virtual_matrix(mumhors_verifier_t *verifier, const int *indices, int num_indices,
    ...
    public_key_row_t* last_pk_row = NULL;
    int last_pk_column = 0;
    int last_pk_i;
    int last_pk_sorted_index;
    ...
    for (int i = 0; i < num_indices; i++) {
        ...
        if (last_pk_row != NULL)
            target_index = sorted_indices[i] - last_pk_sorted_index;
        else
        ...
        int available_valid_pks = pk_row_start->available_pks - pk_row_start->doubt_pks;
        // int available_valid_pks = available_pks;
        int available_doubt_pks = pk_row_start->doubt_pks;
        int column_to_start_searching = 0;
        column_to_start_searching = last_pk_column;

        if (last_pk_row != NULL) {
            get_available_pk_until_index(last_pk_row,last_pk_column, &available_valid_pks, &available_doubt_pks);
        }

        while (pk_row_start) {
            if (target_index < available_valid_pks + available_doubt_pks)
                break;
            target_index -= available_valid_pks + available_doubt_pks;

            cnt_doubt_pk += available_doubt_pks;
            cnt_valid_pk += available_valid_pks;

            pk_row_start = pk_row_start->next;
            column_to_start_searching = 0;

            available_valid_pks = pk_row_start->available_pks - pk_row_start->doubt_pks;
            available_doubt_pks = pk_row_start->doubt_pks;
        }
        ...
        int found_column = 0;
        for (int j = column_to_start_searching; j < verifier->c; j++) {
        ...
        int cnt_doubt_in_between = cnt_doubt_pk;
        int cnt_valid_in_between = cnt_valid_pk;

        if (i!=0) {
            cnt_doubt_in_between = cnt_doubt_pk;
            cnt_valid_in_between = cnt_valid_pk - 1;
        }
        ...
        if (strncmp(target_pk->public_key, sk_hash, SHA256_OUTPUT_LEN) != 0) {
            ...
            if(last_pk_row == NULL)
                printf("Last row 0 col 0 \n");
            else
                printf("Last row %d col %d  sidx %d  \n", last_pk_row->number, last_pk_column, last_pk_sorted_index);

            printf("cnt_doubt_in_between %d cnt_valid_in_between %d\n",cnt_doubt_in_between,cnt_valid_in_between);
            ...
            if ((found_column+1 % verifier->c) ==0){
                found_column=0;
                pk_row_start=pk_row_start->next;
            ...
            int cnt_next_pks_to_check = cnt_doubt_in_between;

            printf("Count forward %d\n\n", cnt_next_pks_to_check);
            int temp_doubt = 0;
            int temp_valid = 0;
            while (pk_row_start) {
                ...
                for (int k = found_column; k < verifier->c; k++) {
                    if (pk_row_start->pks[k]->type != MUM_PK_INVALID) {
                        ...
                        if (pk_row_start->pks[k]->type == MUM_PK_DOUBT)
                            temp_doubt++;
                        else if (pk_row_start->pks[k]->type == MUM_PK_VALID)
                            temp_valid++; //todo maybe -1
                        ...
                        if (strncmp(pk_row_start->pks[k]->public_key, sk_hash, SHA256_OUTPUT_LEN) == 0) {
                            ...
                            temp_valid++; // due to validating the questioned one
                            ...
                            int skipped = cnt_doubt_pk - cnt_next_pks_to_check;

                            printf("New doubt %d new Valid %d\n", temp_doubt, temp_valid);
                            printf("SKIPPED %d\n\n", skipped);

//                                debug_pk(verifier);
//                                printf("\n\n\n");

                            // Invalidate the skipped doubt public keys
                            if (skipped) {
//                                if (last_pk_row != NULL)
//                                    change_pk_status_from_doubt_with_start(verifier, last_pk_row,
//                                                                           last_pk_column, pk_row_start, found_column,MUM_PK_INVALID, skipped);
//                                else
//                                    change_pk_status_from_doubt_with_start(verifier, verifier->pk_matrix.head,
//                                                                           0, pk_row_start, found_column,MUM_PK_INVALID, skipped);

                                cnt_doubt_in_between -= skipped;
                                cnt_valid_in_between += temp_valid;
                            }
                            printf(" doubt %d  Valid %d\n", cnt_doubt_in_between, cnt_valid_in_between);
                            cnt_doubt_in_between += temp_doubt; //todo maybe -1




                            if (last_pk_row != NULL) {
                                if (sorted_indices[i] - last_pk_sorted_index - 1 == 0) {
                                    if (cnt_doubt_in_between > 0) {
                                        // mark as invalid
                                        change_pk_status_from_doubt_with_start(verifier, last_pk_row,
                                                                               last_pk_column, pk_row_start, k,MUM_PK_INVALID, cnt_doubt_pk);
                                    }
                                }
                                else {
                                    if (cnt_doubt_in_between + cnt_valid_in_between <= sorted_indices[i] - last_pk_sorted_index - 1) {
                                        change_pk_status_from_doubt_with_start(verifier, last_pk_row,
                                                                               last_pk_column, pk_row_start, k,MUM_PK_VALID, cnt_doubt_pk);
                                    }
                                }
                                ...
                                if (cnt_doubt_in_between + cnt_valid_in_between <= sorted_indices[i]) {
                                    // mark as valid
                                    change_pk_status_from_doubt_with_start(verifier, verifier->pk_matrix.head,
                                                                           0, pk_row_start, k,MUM_PK_VALID, cnt_doubt_pk);
                                }
                                //todo other condition?
                            }


                            last_pk_row = pk_row_start;
                            last_pk_column = k;
                            last_pk_i = i;
                            last_pk_sorted_index = sorted_indices[i];
...
//                                exit(0);
            ...
            // Check a condition with last verified public key
            printf("cnt_doubt_in_between %d cnt_valid_in_between %d\n\n",cnt_doubt_in_between,cnt_valid_in_between);
            ...
            if (last_pk_row != NULL) {
                if (sorted_indices[i] - last_pk_sorted_index - 1 == 0) {
                    if (cnt_doubt_in_between > 0) {
                        // mark as invalid
                        change_pk_status_from_doubt_with_start(verifier, last_pk_row,
                                                               last_pk_column, pk_row_start, found_column,MUM_PK_INVALID, cnt_doubt_pk);
                    }
                }
                else {
                    if (cnt_doubt_in_between && cnt_doubt_in_between + cnt_valid_in_between <= sorted_indices[i] - last_pk_sorted_index - 1) {
                        change_pk_status_from_doubt_with_start(verifier, last_pk_row,
                                                               last_pk_column, pk_row_start, found_column,MUM_PK_VALID, cnt_doubt_pk);
                    }
                }
            }else {
                if (cnt_doubt_in_between && cnt_doubt_in_between + cnt_valid_in_between <= sorted_indices[i]) {
                    // mark as valid
                    change_pk_status_from_doubt_with_start(verifier, verifier->pk_matrix.head,
                                                           0, pk_row_start, found_column,MUM_PK_VALID, cnt_doubt_pk);
                }
                //todo other condition?
    ...
    for (int i = 0; i < nv2qidx; i++) {
        if (pk_rows_nv2q[i] != NULL) {
            ...
            if (pk_rows_nv2q[i]->pks[pk_index_nv2q[i]]->type == MUM_PK_VALID)
                pk_rows_nv2q[i]->doubt_pks++;
            if (pk_rows_nv2q[i]->pks[pk_index_nv2q[i]]->type == MUM_PK_INVALID) {
                pk_rows_nv2q[i]->available_pks++;
                verifier->active_pks++;
            }
...
int mumhors_verify_signature(mumhors_verifier_t *verifier, const mumhors_signature_t *signature,
     ...
     * through a process known as rejection sampling. */
    int *sorted_indices;
    int verify_status = VERIFY_SIGNATURE_VALID;
    ...
    if (check_rejection_sampling(message, message_len, verifier->k, verifier->t, message_indices, signature->ctr,
        ...
        perform_rejection_sampling(message, message_len, verifier->k, verifier->t, message_indices,
                                   &sorted_indices);
    ...
    int verify_status2 = verify_signature_using_virtual_matrix(verifier, message_indices, verifier->k,
                                                               signature->signature);
    ...
    if (verify_status == VERIFY_SIGNATURE_INVALID) {
        return VERIFY_SIGNATURE_INVALID;
    }else
        return verify_status2;
