                    ...
                    if (pk_row_start->pks[k]->type != MUM_PK_INVALID) {
                        ...
                        if (strncmp(pk_row_start->pks[k]->public_key, sk_hash, SHA256_OUTPUT_LEN) == 0) {
                            ...
                            if (cnt_doubt_pk + cnt_valid_pk == sorted_indices[i]) {
                                ...
                                if (temp_doubt && temp_doubt + i <= skipped)
                                    change_pk_status_from_doubt(verifier, pk_row_start, k, MUM_PK_VALID , temp_doubt);
                            ...
                            // if (cnt_doubt_pk - skipped + temp_doubt + cnt_valid_pk == sorted_indices[i])
                            //     change_pk_status_from_doubt(verifier, pk_row_start, found_column,MUM_PK_VALID, cnt_doubt_pk - skipped + temp_doubt); //todo

                            // change_pk_status_from_doubt(verifier, pk_row_start, k, MUM_PK_VALID , cnt_doubt_pk - skipped + temp_doubt);
