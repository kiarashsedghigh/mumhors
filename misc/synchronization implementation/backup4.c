...
static int verify_signature_using_virtual_matrix(mumhors_verifier_t *verifier, const int *indices, int num_indices,
    ...
    for (int i = 0; i < num_indices; i++) {
                ...
                for (int k = found_column; k < verifier->c; k++) {
                    ...
                    if (pk_row_start->pks[k]->type != MUM_PK_INVALID) {
                        ...
                        if (strncmp(pk_row_start->pks[k]->public_key, sk_hash, SHA256_OUTPUT_LEN) == 0) {
                            ...
                            // printf("\ntotal %d\n",verifier->active_pks);
                            // printf("Dount %d Valid %d index %d temp %d skipped %d i: %d \n",cnt_doubt_pk,cnt_valid_pk,sorted_indices[i],
                                // temp_doubt, skipped, i);
                            // debug_pk(verifier);
                            ...
                            if (skipped + cnt_doubt_pk >=  i) {
                                // change_pk_status_from_doubt(verifier, pk_row_start, k, MUM_PK_VALID , skipped + cnt_doubt_pk);

                                change_pk_status_from_doubt(verifier, pk_row_start, k, MUM_PK_INVALID , skipped);
                                change_pk_status_from_doubt(verifier, pk_row_start, k, MUM_PK_VALID , skipped);

                            }else {
                                change_pk_status_from_doubt(verifier, pk_row_start, k, MUM_PK_VALID , cnt_doubt_pk);
        ...
        accept_signature:
            ...
            if (cnt_doubt_pk && cnt_doubt_pk + cnt_valid_pk == sorted_indices[i])
